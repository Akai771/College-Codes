...
export default function RecipeManagement() {
    ...
    const filteredRecipes = recipes.filter(r => r.name.toLowerCase().includes(searchTerm.toLowerCase()) && (filterCategory === 'All' || r.category === filterCategory));
    ...
    const card = { padding: '10px', border: '1px solid #ddd', borderRadius: '5px' };

    return (
        <div style={{ padding: '20px' }}>
            <h1>Recipe Management System</h1>
            <div style={{ marginBottom: '20px', display: 'flex', gap: '10px' }}>
                <input type="text" placeholder="Search recipes..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: '8px', flex: 1 }} />
            ...
            <div style={{ display: 'flex', gap: '20px' }}>
                <div style={{ flex: 1 }}>
                    {filteredRecipes.map(recipe => (
                        <div key={recipe.id} style={{ ...card, padding: '15px', marginBottom: '10px', cursor: 'pointer', backgroundColor: selectedRecipe?.id === recipe.id ? '#e3f2fd' : 'white' }} onClick={() => setSelectedRecipe(recipe)}>
                ...
                <div style={{ flex: 1 }}>
                    {selectedRecipe ? (
                        ...
                        <div style={{ ...card, padding: '20px', textAlign: 'center', color: '#666' }}>
